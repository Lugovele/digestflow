...
def _build_discovery_cycle_round_summary(
    *,
    ...
    diagnosis = _build_discovery_cycle_round_diagnosis(
        round_result=round_result,
...
def _build_discovery_cycle_payload(
    *,
    ...
    cycle_diagnosis = _build_discovery_cycle_overall_diagnosis(
        decision=decision,
        rounds=rounds,
        accumulated_visible_suggestions=accumulated_visible_suggestions,
    )
...
def _build_discovery_cycle_round_diagnosis(
    *,
    round_result: dict,
    returned_count: int,
    ...
    stale_rejection_count = sum(
        ...
        if isinstance(item, dict) and _is_stale_rejection_reason(str(item.get("reason") or "").strip())
    )
    over_broad_signals = {
    ...
    if visible_new_suggestions >= DISCOVERY_CYCLE_TARGET_VISIBLE_NEW_SUGGESTIONS:
    ...
    secondary_causes = _dedupe_string_list(secondary_causes)

    explanation = _build_discovery_cycle_diagnosis_explanation(
...
def _build_discovery_cycle_overall_diagnosis(
    *,
    decision: str,
    rounds: list[dict],
    accumulated_visible_suggestions: int,
    ...
    if decision == "target_reached":
        return {
            ...
            "severity": "low",
            "explanation": (
                f"The discovery cycle reached the {DISCOVERY_CYCLE_TARGET_VISIBLE_NEW_SUGGESTIONS}-source target "
    ...
    secondary_causes = _dedupe_string_list(
        [
            cause
    ...
    recommended_next_action = _recommended_next_action_for_diagnosis(primary_cause)
...
def _is_stale_rejection_reason(reason: str) -> bool:
...
def _recommended_next_action_for_diagnosis(primary_cause: str) -> str:
    mapping = {
        "target_reached": "stop",
...
def _build_discovery_cycle_diagnosis_explanation(
    *,
    primary_cause: str,
    secondary_causes: list[str],
    provider_error_count: int,
    ...
    primary_label = _format_discovery_cycle_diagnosis_label(primary_cause)
    ...
    if secondary_causes:
        explanation += (
            " Secondary causes: "
            + ", ".join(_format_discovery_cycle_diagnosis_label(item).lower() for item in secondary_causes)
            + "."
        )
    return explanation


def _dedupe_string_list(items) -> list[str]:
    result: list[str] = []
    seen: set[str] = set()
    for item in items or []:
...
def _build_current_research_cycle_summary(cycle: dict) -> str:
    ...
    if cycle_diagnosis:
        ...
        if primary_cause not in {"", "target_reached", "provider_unavailable"}:
            diagnosis_note = f" — {_format_discovery_cycle_diagnosis_label(primary_cause).lower()}."
    if target > 0:
...
def _format_discovery_cycle_diagnosis_label(cause: str) -> str:
    mapping = {
        "provider_unavailable": "Provider unavailable",
...
def _format_discovery_cycle_next_action_label(action: str) -> str:
    mapping = {
        "stop": "Stop",
...
def _build_research_history_cycle_diagnosis(cycle_diagnosis: dict) -> dict:
    ...
    secondary = [
        _format_discovery_cycle_diagnosis_label(str(item).strip())
        for item in cycle_diagnosis.get("secondary_causes") or []
        if str(item).strip()
    ]
    return {
        "has_diagnosis": True,
        "primary_cause": _format_discovery_cycle_diagnosis_label(str(cycle_diagnosis.get("primary_cause") or "").strip()),
        ...
        "recommended_next_action": _format_discovery_cycle_next_action_label(
