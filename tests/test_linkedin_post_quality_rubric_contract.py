...
EXPECTED_SCORING_INVARIANTS = (
    ...
    "The explicit_author_owned_statement_required value is a requirement declaration, not an automatic failure condition; when exactly one qualifying evidence-bounded author-owned interpretive statement is present, author_point_of_view may score 5, failed_criteria must not include author_point_of_view, and automatic_fail_reason must stay empty unless a separate documented automatic failure applies.",
...
class LinkedInPostQualityRubricContractTests(SimpleTestCase):
    ...
    def test_scoring_invariants_capture_false_positive_regressions(self) -> None:
        ...
        for phrase in (
            ...
            "failed_criteria must not include author_point_of_view",
            "clear ownership signal",
            "Ordinary thesis, synthesis",
