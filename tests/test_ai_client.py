...
class AIProviderConfigTests(SimpleTestCase):
    ...
    def test_missing_gemini_key_returns_sanitized_configuration_error(self):
        ...
        self.assertEqual(
            ...
            "semantic grounding provider execution",
        )
    ...
    def test_unsupported_provider_is_rejected_before_client_construction(self):
        self.assertEqual(
            ...
            "unsupported candidate writer provider: unknown",
        )
    ...
    def test_anthropic_provider_rejects_unsupported_claude_models(self):
        ...
        self.assertIn("candidate writer provider/model mismatch", error)
    ...
    def test_openai_provider_rejects_anthropic_model_before_invocation(self):
        self.assertEqual(
            ...
            "semantic grounding provider/model mismatch: provider openai cannot use "
            "an Anthropic model",
        )
    ...
    def test_gemini_provider_rejects_openai_model_before_invocation(self):
        ...
        self.assertIn("semantic grounding provider/model mismatch", error)
        self.assertIn("gemini-3.6-flash", error)
        self.assertNotIn("gemini-2.5-pro", error)
        self.assertNotIn("gemini-2.5-flash", error)
    ...
    def test_openai_provider_rejects_gemini_model_before_invocation(self):
        self.assertEqual(
            ...
            "candidate writer provider/model mismatch: provider openai cannot use "
            "a Gemini model",
        )

    @override_settings(
        ANTHROPIC_API_KEY="anthropic-test-key",
        ANTHROPIC_TIMEOUT_SECONDS=17,
    )
    @patch("apps.ai.client.urlopen")
    def test_anthropic_generation_uses_native_messages_endpoint_and_headers(
        self,
        mock_urlopen,
    ):
    ...
    def test_anthropic_generation_can_disable_thinking_without_changing_max_tokens(
        ...
        mock_urlopen.return_value = _AnthropicResponse(
            {"content": [{"type": "text", "text": '{"post_text": "Draft"}'}], "usage": {}}
        )
        client = build_ai_client("anthropic", "claude-sonnet-5")
    ...
    @override_settings(OPENAI_API_KEY="sk-test", OPENAI_TIMEOUT_SECONDS=30)
    @patch("apps.ai.client.OpenAI")
    def test_openai_rejects_non_default_thinking_mode_before_invocation(
        ...
        with self.assertRaisesRegex(ValueError, "unsupported AI thinking_mode"):
            client.generate_text("Prompt", thinking_mode=THINKING_MODE_DISABLED)
    ...
    def test_anthropic_multiple_text_blocks_are_concatenated_in_order(
        ...
        self.assertEqual(
            result.provider_response_metadata["content_block_types"],
            ["thinking", "text", "text"],
        )

    @override_settings(ANTHROPIC_API_KEY="anthropic-test-key")
    @patch("apps.ai.client.urlopen")
    def test_anthropic_empty_text_blocks_return_empty_text(self, mock_urlopen):
        mock_urlopen.return_value = _AnthropicResponse(
            {
                "content": [{"type": "thinking", "thinking": "audit"}],
                "usage": {
                    "input_tokens": 1,
                    "output_tokens": 1,
                    "thinking_tokens": 1,
    ...
    @override_settings(ANTHROPIC_API_KEY="anthropic-test-key")
    @patch("apps.ai.client.urlopen")
    def test_anthropic_tool_use_only_response_preserves_metadata(self, mock_urlopen):
        mock_urlopen.return_value = _AnthropicResponse(
            {
        ...
        self.assertEqual(result.provider_response_metadata["stop_reason"], "tool_use")

    @override_settings(OPENAI_API_KEY="sk-test", OPENAI_TIMEOUT_SECONDS=30)
    @patch("apps.ai.client.OpenAI")
    def test_openai_compatible_response_metadata_uses_shared_shape(self, mock_openai):
        response = SimpleNamespace(
        ...
        self.assertEqual(
            result.provider_response_metadata,
            {
                ...
                "content_block_types": ["output_text"],
                "input_tokens": 11,
                "output_tokens": 13,
                "thinking_tokens": None,
            },
    ...
    @override_settings(OPENAI_API_KEY="sk-test", OPENAI_TIMEOUT_SECONDS=30)
    @patch("apps.ai.client.OpenAI")
    def test_openai_incomplete_response_metadata_uses_incomplete_reason(
        self,
        mock_openai,
    ):
        response = SimpleNamespace(
    ...
    @override_settings(GEMINI_API_KEY="gemini-test-key", OPENAI_TIMEOUT_SECONDS=30)
    @patch("apps.ai.client.OpenAI")
    def test_gemini_response_metadata_uses_shared_shape(self, mock_openai):
        choice = SimpleNamespace(message=SimpleNamespace(content="Gemini text"))
        response = SimpleNamespace(
            choices=[choice],
    ...
    @override_settings(ANTHROPIC_API_KEY="anthropic-test-key")
    @patch("apps.ai.client.urlopen")
    def test_anthropic_provider_error_is_sanitized_and_not_retried(self, mock_urlopen):
